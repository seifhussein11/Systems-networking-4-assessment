...
def fragment_IP(tcp,client_ip,identification,IP_header_len):
    IP_datagrams =[]
...
# used in simulating the 3 way connection handshake, it receives an IP datagram with the syn flag from the client , then sends an IP datagram with the SYN-ACK flag to the client and wait for the acknowledgment to start the connection .
    ...
    with open(client_log, 'r') as file:
        ...
        if lines:
            ...
            if flag == "SYN":
                ...
                with open(server_log,'a') as log:
                    log.write("Event: Recieved an IP datagram with the SYN flag on and sent an Ethernet datagram with the SYN-ACK flag on\n")
                    log.flush()
                if (len(trial_log_IP_datagram) > 1500):
                    ...
                    with open(server_log,'a') as log:
                        ...
                        log.write("\n")
                        log.flush()
                print("Received an IP datagram including SYN and sent an IP datagram including SYN-ACK")  

    time.sleep(6)  
    # reads the IP datagram containing the ACK flag received from the client and start the connection
    with open(client_log, 'r') as file:
        lines = file.readlines()
        if lines:
            flag = lines[-1].split("Control Flags:")[1].split("|")[0].strip()
...
# receive the IP datagram carrying the HTTP request and return an IP datagram with the HTTP respond to the HTTP request from the client   
def recieve_request_and_handle(request_number):
    with open(client_log,'r') as file:
        lines = file.readlines()
        if lines:
            flag = lines[-1].split("|")[-6].split(":")[1].strip()
            if (flag == "PSH-ACK"):
                request = lines[-1].split("|")[-1].strip()
                last_client_seq_no=lines[-1].split("|")[-9].split(":")[1].strip()
                last_client_ack_no = lines[-1].split("|")[-8].split(":")[1].strip()
                log_packets = handle_client_request(request,last_client_seq_no,last_client_ack_no)
                for log_packet in log_packets:
                    with open(server_log,'a') as log:
                        log.write("\n")                    
                        log.write("Event: IP datagram holding " + (request_number)  + " data\n")
                        log.write(log_packet)
                        log.write("\n")
                        log.flush()
                print("Handeled HTTP request")

# used in simulating the 3 way connection termination handshake, it receives an IP datagram with the FIN flag from the client, then sends an IP datagram with the FIN-ACK flag to the client and close the connection.
    ...
    with open(client_log,'r') as file:
        ...
        if lines:
            ...
            if flag =="FIN":
                ...
                with open(server_log,'a') as log:
                    log.write("\n")
                    log.write("Event: Received an IP datagram with the FIN flag on and sent an IP datagram with the FIN-ACK flag on to close the connection\n")
                    log.flush()
                if (len(trial_log_IP_datagram) > 1500):
                    IP_header_len = len(trial_log_IP_datagram) - len(log_request)
                    IP_datagrams = fragment_IP(log_request,client_ip,identification,IP_header_len) 
                    ...
                    with open(server_log,'a') as log:
                        ...
                        log.write("\n")
                        log.flush()
                print("Received a FIN and sent a FIN-ACK and will close connection")
...
# supporting method used in receive_request_and_handle function, it returns the HTTP response encapsulated inside an IP datagram
def handle_client_request(request,last_seq_no,last_ack_no):
    ...
    if (len(trial_log_IP_datagram)>1500):
        ...
        for datagram in IP_datagrams:
            log_IP_packets.append(datagram)
    else:
        log_IP_packets.append(trial_log_IP_datagram)
    return log_IP_packets
